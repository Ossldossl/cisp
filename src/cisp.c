...
cs_SSAVar gen_let(cs_Context* c)
{
    skip_whitespace(c);
    cs_SSAVar last = ssavar_invalid;
    while (cur() == '(') {
        advance();
        skip_whitespace(c);
        u32 hash = parse_symbol(c);
        if (hash == 0) return ssavar_invalid;
...
void cs_serialize_bb(cs_BasicBlock* bb) {
    ...
    while (!ssa_invalid(cur_phi->dest)) {
